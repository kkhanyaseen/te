...
from bot import dispatcher, DOWNLOAD_DIR, LOGGER, BOT_PM, LEECH_LOG, MAX_SPLIT_SIZE, getLogger
...
def _mirror_leech(bot, message, isZip=False, extract=False, isQbit=False, isLeech=False):
    ...
    if isLeech:
        ...
        if not message.document:
            ...
            # Name
            name_args = message.text.split('|', 1)
            if len(name_args) > 1:
                name = name_args[1]
            ...
            link = link.strip()
            # Password
            pswd_arg = message.text.split(' pswd: ')
            if len(pswd_arg) > 1:
                pswd = pswd_arg[1]
            else:
                pswd = None
        if not link or is_magnet(link):
            isQbit = True
        if message.document:
            message.text = ""
    else:
    ...
    if isLeech:
        file_ = message.document or None
        if file_ is None:
            pass
        elif isinstance(file_, list):
            link = file_[-1].get_file().file_path
...
qb_zip_mirror_handler = CommandHandler(BotCommands.QbZipMirrorCommand, qb_zip_mirror,
                                filters=CustomFilters.authorized_chat | CustomFilters.authorized_user, run_async=True)
auto_leech_handler = MessageHandler(CustomFilters.detect_torrent_and_magnets & (filters.Filters.text | filters.Filters.document), 
                                leech, run_async=True)
...
dispatcher.add_handler(auto_leech_handler)
