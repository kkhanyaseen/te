...
class MirrorLeechListener:
    def __init__(self, bot, message, isZip=False, extract=False, isQbit=False, isLeech=False, pswd=None, tag=None, select=False, seed=False):
        ...
        self.suproc = None
        self.user_id = self.message.from_user.id if self.message.chat.type != 'channel' else self.message.chat.id
        ...
        if self.isLeech:
            ...
            if MIRROR_LOGS:
                try:
                    ...
                    LOGGER.warning(e)
            if BOT_PM and self.message.chat.type not in ('private', 'channel'):
                try:
